...
/**
 * หัวข้อแจ้งเตือนที่ผู้ดูแลระบบตั้งค่าได้
 *
 * เหมือนฝั่งผู้จัดกิจกรรมตรงที่ไม่มีสวิตช์ของ "งานค้าง" ให้ปิด — คำขอลบบัญชี
 * และข้อความถึงผู้ดูแลคำนวณสดจากสถานะจริงเสมอ ปิดได้ก็เท่ากับซ่อนงานที่ต้องทำ
 */
const TOGGLES: { key: ToggleKey; label: string; desc: string; icon: string }[] = [
  { key: 'systemNotice', label: 'ประกาศจากระบบ', desc: 'ข่าวสารและประกาศปิดปรับปรุงจากระบบ', icon: 'campaign' },
  { key: 'chatMessage', label: 'ข้อความจากผู้ใช้', desc: 'แจ้งเตือนเมื่อมีผู้ใช้ทักเข้ามาในห้องแชท', icon: 'forum' },
  { key: 'emailEnabled', label: 'ส่งอีเมลด้วย', desc: 'สถานะการส่งอีเมลจริงดูได้ที่หน้าการเชื่อมต่อระบบ', icon: 'mail' },
];
    ...
    <div style={{ display: 'grid', gap: 16, animation: 'nuFadeUp .3s ease' }}>
      ...
      <Section icon="info" title={t('ข้อมูลระบบ')}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 12 }}>
          ...
          <Fact icon="shield" label={t('บทบาท')} value={t('ผู้ดูแลระบบ')} />
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 14 }}>
          <Link href="/admin/ops">
            <Button variant="secondary" icon="hub">
              {t('ดูสถานะการเชื่อมต่อระบบ')}
            </Button>
          </Link>
          <Link href="/admin/logs">
            <Button variant="secondary" icon="receipt_long">
              {t('เปิด System Log')}
            </Button>
          </Link>
        </div>
    ...
    >
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 14 }}>
        ...
        <Badge
          ...
          icon="block"
          label={`${t('บัญชีที่ถูกระงับ')} ${pendingWork.suspended}`}
        />
        <Link href="/admin/notifications" style={{ alignSelf: 'center', fontSize: 12.5, color: COLOR.link }}>
